...
static void send_packet(void *ptr) {
    ...
    PRINTF("Un objet vient de rentrer \n");
    leds_on(LEDS_RED);
    sprintf(buf, "%i", 1);
...
static struct etimer timer_light;
...
etimer_set(&timer_light, 180 * CLOCK_SECOND);
...
if(etimer_expired(&timer_light)){
  leds_off(LEDS_BLUE);
  pair = pair + 1;
  etimer_reset(&timer_light);
  if (pair % 2 == 0){
    leds_on(LEDS_BLUE);
    if (light>0){
      light = -80;
...
if(etimer_expired(&periodic) && light<=0) {
